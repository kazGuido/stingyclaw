...
import { createTempDir, setupStingyclawDir, initGitRepo, cleanup } from './test-helpers.js';
...
describe('resolution-cache', () => {
  ...
  it('saveResolution writes file_hashes to meta.yaml', () => {
    ...
    const resDir = path.join(tmpDir, '.stingyclaw', 'resolutions', 'alpha+beta');
  ...
  describe('loadResolutions hash verification', () => {
    ...
    function setupInputFiles() {
      ...
      fs.writeFileSync(path.join(tmpDir, '.stingyclaw', 'base', 'src', 'config.ts'), baseContent);
