...
import { BASE_DIR, CUSTOM_DIR, STINGYCLAW_DIR } from './constants.js';
import { initStingyclawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initStingyclawDir();
  console.log('Skills system initialized. .stingyclaw/ directory created.');
}
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      const filesModified = [...diff.matchAll(/^diff -ruN .+ (.+)$/gm)]
        ...
        .filter((f) => !f.startsWith('.stingyclaw'));
      ...
      console.log(
        'Custom modifications captured in .stingyclaw/custom/migration.patch',
      );
    } else {
