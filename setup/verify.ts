...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.stingyclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.stingyclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active stingyclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('stingyclaw')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(envFile)) {
    ...
    if (/^(OPENROUTER_API_KEY|ANTHROPIC_API_KEY)=/m.test(envContent)) {
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'stingyclaw', 'mount-allowlist.json'))) {
