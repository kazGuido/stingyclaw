...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // (group folder, IPC, .stingyclaw/) are mounted separately below.
  ...
  // Sync skills from container/skills/ into each group's session dir
  ...
  mounts.push({
    ...
    containerPath: '/home/node/.stingyclaw',
    readonly: false,
  });
  ...
  // Syncs on every startup so code changes in container/agent-runner/src/
  // always reach existing groups without manual intervention.
...
function readSecrets(): Record<string, string> {
  return readEnvFile(['OPENROUTER_API_KEY', 'MODEL_NAME', 'OPENROUTER_BASE_URL']);
}
...
export async function runContainerAgent(
  ...
  const containerName = `stingyclaw-${safeName}-${Date.now()}`;
